...
"use strict";
/**
 * dsh-streamfold —— 流式折叠（DSH Web 显示增强）
 *
 * 模型只有两条：
...
const NATIVE_LABELS_EN = { compact: "Compact", standard: "Standard", detailed: "Detailed", verbose: "Verbose", fold: "Fold" };
let tChat = null;   // 官方 chat 词典：标签就跟官方设置页一致，改动由官方那边带过来
/** 界面语言：官方词典 zh 里 compact 是「简洁」、en 里是 Compact。
 *  官方**没有** fold 键，所以「折叠」这一档和设置页的两处标题必须自带双语，不能只靠 tChat 回退。 */
const isZh = () => {
  if (!tChat) return true;
  try { return tChat("settings.transcript.compact") === NATIVE_LABELS.compact; } catch { return true; }
};
const uiText = (zh, en) => (isZh() ? zh : en);
const modeLabel = (mode) => {
  const fallback = (isZh() ? NATIVE_LABELS : NATIVE_LABELS_EN)[mode] || mode;
...
const CSS = [   // 本文件的样式表字符串（名字遮蔽了浏览器全局 CSS 对象，本插件已不再用 CSS.highlights）
  ...
  "[data-dshsf-answer-sep]{border-top:1px solid var(--dsw-alias-border-l4,rgba(127,127,127,.38));padding-top:6px}",
  ...
  "[data-dshsf-window]::-webkit-scrollbar{width:8px;height:8px}",
  "[data-dshsf-window]::-webkit-scrollbar-thumb{background:var(--dsw-alias-scrollbar-bg-l2,rgba(128,128,128,.35));border-radius:4px}",
  "[data-dshsf-window]::-webkit-scrollbar-track{background:transparent}",
  ...
  "[data-dshsf-jump]{position:sticky;bottom:6px;float:left;z-index:3;display:inline-flex;align-items:center;justify-content:center;gap:4px;",
  "margin:0 0 -27px 6px;height:27px;box-sizing:border-box;",   // 负下边距：窗口是 BFC，会把浮动包进高度 —— 不抵消每个窗底下白多 27px
  ...
  "opacity:0;pointer-events:none;transform:translateY(4px);transition:opacity .2s ease,transform .2s ease}",   // 藏起来时不占位也不挡正文点击
  "[data-dshsf-jump][data-dshsf-visible]{opacity:1;pointer-events:auto;transform:translateY(0)}",
  /* 主窗口那颗回底按钮：外观与位置都用官方的，只把"何时出现"接管过来——默认藏起来，
     只有我们判定"读者接管了、而且确实离底"时才放出来（官方自己按 25px 阈值无滞回地切 = 闪）。 */
  "html[data-dshsf-mode='fold'] [class*='_toBottomSlot']{display:none}",
  "html[data-dshsf-mode='fold'][data-dshsf-jumpgate] [class*='_toBottomSlot']{display:flex}",
  ...
  "@media (prefers-reduced-motion: reduce){[data-dshsf-window],[data-dshsf-row],[data-dshsf-chip],[data-dshsf-jump],.dshsf-switch,.dshsf-switch:after{transition:none!important;animation:none!important}}",
].join("");
...
function rootVars() {
  ...
  // 非折叠模式 = 插件整体撒手：样式也按模式分支（官方那颗回底按钮只在折叠模式由我们接管显隐），
  // 动效属性同样只在折叠模式挂。
  const fold = state.mode === "fold";
  document.documentElement.setAttribute("data-dshsf-mode", fold ? "fold" : "native");
  document.documentElement.toggleAttribute("data-dshsf-anim", fold && state.animations !== false && allowAnim);
}
...
function animateClose(el, done) {
  ...
  setTimeout(() => { if (disposed) return; if (typeof done === "function") done(); finishClose(el); }, 200);
}
/** 合成 click 没能让 React 收起时（控件被换掉/被拦下），把"正在收"的标记与内联样式还原 ——
 *  否则这条正文会永久停在 inline max-height:0 + opacity:0，revealBody/帧循环都按 closing 早退、再也不碰它。 */
function finishClose(body) {
  if (!body || !body.isConnected || body.dataset.dshsfClosing !== "1") return;
  delete body.dataset.dshsfClosing;
  body.style.maxHeight = ""; body.style.opacity = ""; body.style.overflow = "";
}
/** 每个模块实例一个接线令牌：热重载后新实例必须能重新挂自己的监听（见 wireDisclosure）。 */
const WIRE_TOKEN = "wire-" + Math.random().toString(36).slice(2, 8);
/** 用户点收起时先播动画再放行：捕获阶段拦下 React 的 onClick。 */
function wireDisclosure(root, isOpen, bodySel, onUserClose) {
  // 认亲而不是认「已接线」：写死 "1" 时，旧实例留下的标记会把新实例挡在门外 ——
  // 旧 handler 已被 disposed 挡住、新 handler 又挂不上 = 收起动画拦截静默失效到刷新。
  if (root.dataset.dshsfWired === WIRE_TOKEN) return;
  root.dataset.dshsfWired = WIRE_TOKEN;
  root.addEventListener("click", (event) => {
    if (disposed || synthetic || !isOpen(root)) return;   // 卸载/热重载后不再拦点击                       // 我方事件 / 是展开动作 → 放行
    if (event.target instanceof Element && event.target.closest(bodySel)) return;   // 点在正文里 → 放行
    if (state.mode !== "fold" || state.animations === false || !allowAnim) return;   // 非折叠档 / 关了动效：直接放行
...
let textCache = new WeakMap();   // 行 → { turn, value }：有没有正文。跨扫描保留，但**必须认轮号** ——
                                 // React 会把同一个行节点复用给别的轮次/别的会话，只认节点会把旧结论套到新内容上
                                 // （缓存说「没正文」时，承载正式回答的行会被当过程行折掉）。切会话时整表重建。
...
function hasText(row) {
  ...
  if (!hotRows.has(row)) {
    ...
    if (hit !== undefined && hit.turn === (row.getAttribute("data-chat-turn") || "")) return hit.value;   // 老轮次：正文不会再变，直接复用（轮号对不上＝节点被复用，重算）
  }
  ...
  textCache.set(row, { turn: row.getAttribute("data-chat-turn") || "", value: result });
  return result;
}
...
function collapse(root, isOpen, bodySel, atKey, triesKey) {
  ...
  if (!target) return false;
  const body = bodyOf(root, bodySel);
  const canAnim = !!(body && state.animations !== false && allowAnim && animBudget > 0 && body.dataset.dshsfClosing !== "1");
  lastClose = {   // 诊断：这次收起为什么播/没播动画（probe().anim.lastClose）
    at: Math.round(performance.now()), anim: canAnim, kind: bodySel === THINK_BODY ? "think" : "tool",
    setting: state.animations !== false, allowAnim, budget: animBudget, tries,
    body: body ? 1 : 0, closing: body && body.dataset.dshsfClosing === "1" ? 1 : 0,
  };
  if (canAnim) {
    animBudget -= 1;
    layoutReads += 1;
    // 只写一处：dataset 落到 DOM 上就是 data-dshsf-closing="1"（CSS 按"属性在不在"匹配）。
    // 再补一句 setAttribute(…, "") 会把值改成空串，下面所有 `=== "1"` 的守卫全部失效
    // （tickFollow 不再给收起的窗让路、revealBody/wireDisclosure 也认不出"正在收"）——
    // 帧循环每帧把 max-height 写回内容高度，收起过渡被反复拽回去：自动折叠只剩淡出、高度不动。
    ...
    setTimeout(() => {
      if (disposed) return;   // 卸载后别再派发合成 click（拦截器有守卫，这一次没有）
      const late = disclosureTarget(root);
      if (late) clickOnce(late);
      finishClose(body);   // 合成 click 没能让 React 收起时别把窗留在"透明且 0 高"
    }, 200);
    return true;
  }
...
function closeThink(root) {
  ...
  const ok = collapse(root, thinkOpen, THINK_BODY, "dshsfCloseAt", "dshsfCloseTries");
  if (ok) autoThinks.delete(root);   // 真收起来了才注销登记：失败（控件被换掉/被节流/tries 用满）时留着，下一轮还能重试
  return ok;
}
function closeTool(root) {
  if (!toolOpen(root)) { delete root.dataset.dshsfToolAuto; autoTools.delete(root); return false; }
  if (root.dataset.dshsfToolAuto !== "1") { autoTools.delete(root); return false; }   // 用户自己开的，不动
  const ok = collapse(root, toolOpen, TOOL_BODY, "dshsfToolCloseAt", "dshsfToolCloseTries");
  if (ok) autoTools.delete(root);    // 同上：收起失败要留着重试，否则旧窗整轮都敞着
  return ok;
}
/** 展开后按实测高度播揭示动画：固定 2000px 上限对矮卡片等于"瞬间全开"，只有量到真实高度才看得见展开。 */
let lastReveal = null;
let lastWindow = null;
let lastClose = null;   // 最近一次"收起一行/一个窗"的动画决策（诊断用）
...
function revealBody(body, preHeight) {
  ...
  if (body.parentElement && body.parentElement.closest("[data-dshsf-window]")) {
...
function scheduleSupersedeClose(root, ms) {
  ...
  root.dataset.dshsfCloseTimer = String(setTimeout(() => {
    ...
    if (state.mode !== "fold") return;                                   // 已切到官方档位：别再动用户的窗口
    ...
    else delete root.dataset.dshsfSupersededAt;   // 这次没收起（控件被换掉/被节流）：撤掉"已排定时器"标记，让下一轮扫描重排
  }, Math.max(0, ms)));
}
...
function jumpButton() {
  ...
  btn.setAttribute("aria-label", uiText("回到底部", "Back to bottom"));
  btn.title = uiText("回到底部", "Back to bottom");
...
function wireWindow(el, followBottom = true) {
  ensureWindowJump(el);
  liveWindows.add(el);
  if (wired.has(el)) return;
  wired.add(el);
  // 穿插正文窗不跟底：静态段落跟到底只会看到"最后一行 + 一屏空白"（内容短时浏览器会把 scrollTop 夹回 0）
  el.dataset.dshsfFollow = followBottom ? "1" : "";
  const noteIntent = () => { if (disposed) return; el.dataset.dshsfIntentAt = String(performance.now()); };
...
function tagThinkWindow(body, preHeight) {
  ...
  if (body.parentElement && body.parentElement.closest("[data-dshsf-window]")) return;
  ...
  for (const item of plan) {
    if (item.want !== null) {
      ...
      if (Math.abs(gap) > 0.5) {
        ...
        if (known === undefined) item.el.dataset.dshsfH = String(item.want);
        // 自愈：内联 max-height 可能停在旧值或被 React 重渲染抹掉（dataset 记的是目标值）—— 只比字符串、
        // 不读布局。不补这一刀，窗口就会一直停在旧高度（用户截图里那种"半空的玻璃面板"）。
        const target = item.want + "px";
        if (item.el.style.maxHeight !== target) { item.el.style.maxHeight = target; moving = true; }
      }
    }
    ...
    if (item.bottom && item.el.dataset.dshsfTextwindow !== "1" && item.el.dataset.dshsfFollow !== "1") item.el.dataset.dshsfFollow = "1";   // 正文窗永不自开跟底（跟底 = 只看得到尾巴）
...
function syncSession() {
  ...
  // 官方把 data-conversation-session 挂在滚动容器的**父层**（div.body[data-conversation-content]），
  // 在 scroll 元素内部 querySelector 恒为 null —— 必须从自己往上找，否则退化成 document 序第一个，
  // 右栏子代理会话排在前面时「切主会话」根本不复位。
  const holder = (scope && scope.closest ? scope.closest("[data-conversation-session]") : null)
    || scope.querySelector("[data-conversation-session]")
  ...
  sessionKey = key;
  manualState.clear();
  snapOwn = false;              // 换会话：位置所有权归零（新会话的首次贴底让官方做）
  glide = null;                 // 滑行 / 钉住补偿 / 记下的写入位置全是上一会话的：留着会继续把新会话的视角拖走
  anchorStop();
  followWrote = null;
  announcedTop = -1;
  announcedHeight = -1;
  ...
  lastRunningAt = 0;
  runningSticky = false;
  // 换会话必须把上一会话的「收尾折叠 hold」清掉：data-chat-turn 每会话都从 1 起，
  // 留着它会让新会话里同号的历史轮被当成"刚跑完那轮"——那不是不折，是主动展开（走 toUnfoldAll）。
  settleHoldTurn = null;
  settleHoldUntil = 0;
  textCache = new WeakMap();   // 换会话：行节点会被复用，正文缓存整表作废
...
function skipSoon(row) {
  setTimeout(() => {
    ...
    setSkipHidden(row);
    for (const el of row.querySelectorAll("[data-dshsf-window]")) liveWindows.delete(el);   // 不渲染的窗不必每帧跟进
  }, 320);
}
function applyFold(row, fold, animate) {
  ...
  if (!animate || (animBudget <= 0 && !scanAnimateAll)) {   // 配额用完 → 直接切；用户手点那次（scanAnimateAll）不看配额，展开方向也得看得见
  ...
  // 非点击展开（Ctrl+F 命中 / 程序化展开 / 关掉「折叠历史轮次」）也要把窗接回每帧清单：
  // 否则它停在折叠前的内联 max-height，不回底、没阴影 —— 就是「半空的玻璃面板」。
  if (!disposed) for (const el of row.querySelectorAll("[data-dshsf-window]")) wireWindow(el, el.dataset.dshsfFollow !== "");
...
function foldBatch(rows, animate) {
  ...
  const flat = !animate || (targets.length > ANIM_BUDGET && !scanAnimateAll);
  noteFold({ at: Math.round(performance.now()), rows: targets.length, animate: !!animate, flat, budget: ANIM_BUDGET, all: scanAnimateAll, hold: settleHoldTurn, gliding: glide !== null });
  if (flat) { for (const row of targets) applyFold(row, true, false); return; }
  layoutReads += 1;
...
function unfoldAll(container) {
  for (const row of container.querySelectorAll("[data-dshsf-row]")) {
    clearSkipHidden(row);
    if (!disposed) for (const el of row.querySelectorAll("[data-dshsf-window]")) wireWindow(el, el.dataset.dshsfFollow !== "");   // 同上：展开后接回每帧清单（卸载中不接线）
    row.removeAttribute("data-dshsf-folded");
    delete row.dataset.dshsfCards;
    row.style.maxHeight = "";
    row.style.overflow = "";
...
let searchPrepTimer = 0;     // 分片挂载的定时器句柄：卸载/热重载要能停掉，否则剩余分片继续展开+藏正文
...
function searchPrep() {
  if (disposed || searchPrepRunning || state.mode !== "fold") return;   // 官方档位不接管 Ctrl+F
  const flow = chatScope();
  ...
  const chunk = () => {
    searchPrepTimer = 0;
    if (disposed) { searchPrepRunning = false; return; }   // 卸载/热重载：剩余分片不许再动 DOM
    let done = 0;
    ...
    if (cursor < roots.length) searchPrepTimer = setTimeout(chunk, 0);
...
function syncTurns(flow, running, collected) {
  ...
  for (const [turn, turnRows] of turns) {
    ...
    // 刚跑完的那一轮先不折：等回到提问处的滑行结束再折（probe().settleLog 能看到这条 hold）。
    const heldForSettle = settleHoldTurn !== null && String(turn) === settleHoldTurn;
    const shouldFold = !heldForSettle && (manual === "fold"
    ...
    for (const row of turnRows) {
      ...
      // 冷行（不在最近两轮）+ 已经折着 → 内容既不变也不渲染，没什么可收的：跳过整棵子树查询。
      // （实测 18 轮时每扫描 37 次 querySelectorAll(THINK) ≈ 2N+1，长会话下是 phases.turnRows 的主要成分。）
      if (!isRunningTurn && rowKind(row) === "assistant-step" && !(fold && isFolded && !hotRows.has(row))) {
    ...
    if (!chip) {
      ...
      chip.addEventListener("click", () => {
        ...
        allowAnim = true;
        scanAnimateAll = true;                                  // 用户手点的折叠：即使整轮一起折也要看得到动效
        releaseSettleHold();                                    // 用户的点击压过「刚跑完那轮先别折」的 hold：否则这一下会被吞掉一次扫描
    ...
    const summary = [];
    // 量纲：thinks 是思考**块**数（一轮可能多段），不是轮数；文案一律走 uiText，英文界面不再露中文
    if (thinks > 0) summary.push(uiText(thinks + " 段思考", thinks + " thinking block" + (thinks > 1 ? "s" : "")));
    if (tools > 0) summary.push(uiText(tools + " 次工具调用", tools + " tool call" + (tools > 1 ? "s" : "")));
    const label = folded > 0
      ? uiText("已折叠 " + (summary.length ? summary.join(" · ") : folded + " 行") + " · 点击展开",
               "Collapsed " + (summary.length ? summary.join(" · ") : folded + " rows") + " · click to expand")
      : uiText("收起本轮过程", "Collapse this turn");
...
function anchorSync() {
  if (disposed) { anchorStop(); return false; }   // 卸载后旧实例不许再写 scrollTop
...
function anchorDuring(node, before, ms) {
  ...
  if (!scroller) return;
  glide = null;        // 点开/收起 = "我要读这一处"：一次性滑行必须让位（否则它每帧把视角拽向旧目标）
  releaseSettleHold();
  anchorStop();
...
// 本次扫描是否属于「用户正看着的折叠」（点折叠条 / 跑完后的收尾折叠）：这类即使目标多也要播动画；
// 加载/切会话那种批量收尾仍走瞬时（见 ANIM_BUDGET 的注释）。
let scanAnimateAll = false;
...
/** 非折叠模式 = 插件整体撒手：清掉我们加过的标记/自绘控件/每帧清单，并把滚动接管还给官方。 */
function standDown(flow) {
  if (flow) { unfoldAll(flow); clearChips(flow); }
  for (const el of document.querySelectorAll("[data-dshsf-window]")) {
    el.removeAttribute("data-dshsf-window");
  ...
  for (const el of document.querySelectorAll("[data-dshsf-search]")) { el.removeAttribute("data-dshsf-search"); clearSkipHidden(el); }   // 归属感知：别把官方挂的那份也摘了、也别留僵尸标记
  for (const el of document.querySelectorAll("[data-dshsf-jump]")) { try { el.remove(); } catch { /* ignore */ } }
  liveWindows.clear();          // 不再每帧跟这些小窗（autoThinks/autoTools 留着：切回折叠时还要靠它们收旧窗）
  pinRows = []; pinRow = null; pinPick = -1;
  hidePinBar();
  releaseTakeover();
  sparks.focus(null);
  sparks.forge(null);
}
/** 归还滚动接管：官方档位下 scrollTop 归官方（toBottom / followRef），回底按钮也由官方自己管。 */
function releaseTakeover() {
  snapOwn = false;              // 位置所有权一起还回去（否则切回官方档后官方贴底仍被挡）
  if (snapTaken !== null) { try { delete snapTaken.el.scrollTop; } catch { /* ignore */ } snapTaken = null; }
  if (document.documentElement.hasAttribute("data-dshsf-jumpgate")) document.documentElement.removeAttribute("data-dshsf-jumpgate");
  glide = null;
  settleHoldTurn = null; settleHoldUntil = 0;   // 上一轮的收尾 hold 一起作废（同「换会话」：轮号各会话都从 1 起，会撞）
}
function mainFollowTick(dt) {
  const scroller = mainScroller();
  if (!scroller) { hidePinBar(); return false; }   // 不在会话视图（设置页/别的面板）：body 上的固定层不能留着
  // 官方档位 = 插件整体撒手：不跟随、不补发 scroll、不钉提问、不接管官方按钮（帧循环随之自然停）。
  if (state.mode !== "fold") { releaseTakeover(); hidePinBar(); return false; }
  ...
  takeOverSnap(scroller);   // 幂等（同实例直接返回）：从官方档切回来时要把 setter 重新装上

  const sh = scroller.scrollHeight;                           // 每帧只读一次：DOM 脏时每次读都强制一次重排
  followHeight = sh;                                          // scroll 事件据此判断"是布局长高了还是有人在跳"
  pinTick(scroller);                                          // 置顶条：读一遍行/容器位置并摆好（固定层，不改会话布局）
  const floor = Math.max(0, sh - scroller.clientHeight);
  ...
  const top0 = scroller.scrollTop;                            // 抢跑检测读一次，下面回补后再重读一次就够
  if (followOn && followWrote !== null && followWrote < floor - 1 && top0 > followWrote + 1) {
    stealCount += 1;
    const px = Math.round(top0 - followWrote);
    if (px > stealMax) stealMax = px;
    stealTail.push(px);
    if (stealTail.length > 8) stealTail.shift();
    writeTop(scroller, followWrote); undoCount += 1;   // 跟随中一律回补（原先「离底 ≤600px 才补」会把单帧长高 >600px 的那一下放弃）
  }
  const top = scroller.scrollTop;                              // 回补可能改过位置 → 重新读一次，后面全部复用
  const gap = floor - top;
  // 恢复跟随要分清"这个位置是谁放上去的"：只有真贴底（gap<=2），或位置就是我方上一次写入，
  // 才算"贴底恢复"。读者自己滚到离底 25px 以内时若直接 followOn=true，下一帧就把人拽回底部 ——
  // 那正是"滚动跟触底吸附打架"。不恢复也没关系：滚到真贴底会自动接上，点回底按钮也随时能接。
  const mine = followWrote !== null && Math.abs(top - followWrote) <= 2;
  // 滑行期间（回提问处 / 点置顶条）**不许**把跟随重新打开：滑行第一帧位置还在底部、mine 也成立，
  // 会把刚取消的 followOn 又顶回 true —— 等滑到提问处，跟随立刻把人拽回底部，看着就是「回提问处失效」。
  // 所以「先取消触底吸附」必须连这个再武装口一起关掉。
  if (!gliding && gap <= 25 && (gap <= 2 || mine)) followOn = true;
...
function onMainScroll(event) {
  ...
  if (followWrote === null || Math.abs(el.scrollTop - followWrote) > 2) {   // 位置不是我写下的 → 不是我们自己造成的
    ...
    if (performance.now() - scrollIntentAt <= USER_SCROLL_MS || el.scrollHeight === followHeight) {
      ...
      followOn = false;
      glide = null;   // 读者明确接管：一次性滑行（每帧都会写回自己的目标）必须一起取消，否则滚了也被拖走
      releaseSettleHold();
    }
...
function chatFlow() {
  ...
  // 右栏子代理（embedded）会话挂在 [data-sidebar-chat] 里（内核 dsh-client-ui-subagent 的 SidebarChatTab）。
  // 文档序第一个 [data-chat-flow] 未必是主会话 —— 优先取不在 sidebar 里的那一个，一个都没有才退回第一个。
  const all = document.querySelectorAll(CHAT_FLOW);
  flowCache = null;
  for (const el of all) {
    if (el.closest("[data-sidebar-chat]") === null) { flowCache = el; break; }
  }
  if (flowCache === null) flowCache = all.length ? all[0] : null;
  return flowCache;
}
...
const settleLog = [];        // 诊断：每次「跑完回到提问处」的判定（为什么没滑 / 滑到哪），probe().settleLog 可读
const foldLog = [];          // 诊断：每次批量折叠的目标数与走的是动画还是瞬时，probe().foldLog 可读
function noteFold(entry) { foldLog.push(entry); if (foldLog.length > 10) foldLog.shift(); }
function noteSettle(entry) { settleLog.push(entry); if (settleLog.length > 8) settleLog.shift(); }
let settleHoldTurn = null;   // 刚跑完的这一轮先别折：等「回到提问处」的滑行走完再折（折叠会改布局，边走边折会把人带偏）
let settleHoldUntil = 0;     // 兜底时限：滑行若被别处取消/丢失，最多拖到这时刻
/** 放行被压住的那一轮折叠，并排一次扫描去折。 */
function releaseSettleHold() {
  if (settleHoldTurn === null) return;
  settleHoldTurn = null; settleHoldUntil = 0;
  scanAnimateAll = true;    // 跑完那轮的收尾折叠是用户正看着的：放行时让它播动画
  schedule();
}
...
function lastUserRow(rows) {
  // 传进来的 collected.rows 是**全部**行：必须自己筛出用户行。
  // （0.5.2 那批「复用行快照」优化里漏了这一步，直接把最后一行当提问行 —— 于是滑行目标是回答行，
  //   表现就是「跑完回到提问处」失效。）
  const list = rows ? rows.filter((el) => el.getAttribute("data-chat-flow-kind") === "user") : userRows();
  return list.length === 0 ? null : list[list.length - 1];
}
/** 本轮跑完：平滑移到本轮提问处。关掉开关、读者已接管滚动、或找不到提问行时都不动。 */
function settleToPrompt(flow, rows) {
  // 每个出口都记一条：probe().settleLog 直接说明「跑完」这次为什么没滑（设置关 / 读者接管 / 没容器 / 没提问行）
  const mark = (why, extra) => noteSettle(Object.assign({ at: Math.round(performance.now()), why, followOn, autoFollow: state.autoFollow !== false, mode: state.mode }, extra || {}));
  if (state.settleToPrompt === false) return mark("setting-off");
  const scroller = mainScroller();
  if (scroller === null) return mark("no-scroller");
  // 中途滚过不管：只看「结束这一刻」是不是还在触底跟随状态（跟随开着，或本来就贴着底）。
  const gap = Math.max(0, scroller.scrollHeight - scroller.clientHeight - scroller.scrollTop);
  if (!(followOn || gap <= 25)) return mark("not-at-bottom", { gap: Math.round(gap) });
  const row = lastUserRow(rows);
  if (row === null) return mark("no-user-row", { rows: rows ? rows.length : -1 });
  ...
  if (state.animations === false) { writeTop(scroller, want); return mark("jump", { from, want, floor, turn }); }   // 动效关掉 → 直接到位
  glide = { el: row, until: performance.now() + SETTLE_MAX_MS, t0: performance.now(), from, want };
  if (turn !== null && turn !== "") { settleHoldTurn = String(turn); settleHoldUntil = performance.now() + SETTLE_MAX_MS + 200; }   // 先回提问处，滑完再折
  ensureTickLoop();
  mark("glide", { from, want, floor, turn });
}
...
function glideTick(scroller, floor, dt) {
  ...
  // 注意：滑行结束时要把 followWrote 一起清掉。滑行每帧都写 followWrote，收尾那帧 gliding 已经是 false，
  // 于是「贴底恢复跟随」判据里的 mine 仍为真 —— 目标离底 ≤25px 时滑完会被重新武装跟随、又吸回底部。
  if (!glide.el.isConnected || performance.now() > glide.until) {
    noteSettle({ at: Math.round(performance.now()), why: glide.el.isConnected ? "glide-timeout" : "glide-el-gone", from: glide.from, want: glide.want, now: Math.round(scroller.scrollTop), ms: Math.round(performance.now() - (glide.t0 || 0)) });
    glide = null; followWrote = null; releaseSettleHold(); return false;   // 滑行结束不再认领跟随写入点，见下
  }
  const want = Math.min(floor, Math.max(0, viewTop(scroller, glide.el) - SETTLE_PAD));
  const dy = want - scroller.scrollTop;
  if (Math.abs(dy) <= 0.5) { noteSettle({ at: Math.round(performance.now()), why: "glide-done", from: glide.from, want, now: Math.round(scroller.scrollTop), ms: Math.round(performance.now() - (glide.t0 || 0)) }); glide = null; followWrote = null; releaseSettleHold(); return false; }
...
function ensurePinBar() {
  ...
  bar.addEventListener("click", () => {
    ...
    glide = { el: pinRow, until: performance.now() + SETTLE_MAX_MS, t0: performance.now(), from: Math.round(el.scrollTop), want: null };
    ensureTickLoop();
  });
...
function onToBottomClick(event) {
  ...
  if (btn === null || btn.closest("[data-dshsf-window]") !== null) return;   // 小窗里的按钮不是它
  // 官方档位 / 关动效 / 关跟随：这颗按钮完全归官方。先交还位置所有权，否则它自己的瞬跳会被我们的接管挡掉（点了没反应）
  if (state.mode !== "fold" || state.autoFollow === false || state.animations === false) { snapOwn = false; return; }
  event.preventDefault();
  event.stopPropagation();          // React 的 onClick 在根上冒泡阶段，捕获阶段拦得住
  followOn = true;
  glide = null;                     // 点了回底就是要到底：别让没跑完的滑行继续往旧目标带
  releaseSettleHold();
 ...
 * 看着就是闪）。所以在滚动容器实例上装一个 setter：**只要位置还归我们管（snapOwn），就把这种"一步贴到底"
 * 的写入直接丢掉**，位置从此只有我们一个写入者；读者/导航/换会话的写入照旧放行。
 * 判据必须是"状态"而不是"我方最近写过"：停在底部时帧循环本来就停了 —— 正是这段没写的空档里，输出一恢复，
 * 官方那条 toBottom 就把一整块新内容瞬移到位（用户看到的就是"新内容一出现就被吸到底"）。
 * 撒手（读者接管 / 切官方档 / 换会话 / 放行官方按钮）时所有权立刻归还，官方行为原样恢复。
 */
let ownWrite = false;        // 我方写入：setter 直接放行
let snapOwn = false;         // 贴底位置归不归我们管。不能用"最近 800ms 写过"当判据：停在底部时帧循环本来就停了，
                             // 这一停就把接管判据晃没了——输出一停顿，下一个 chunk 到达时官方的瞬跳正好从空档漏过去。
...
function writeTop(el, value) {
  ...
  if (followOn || glide !== null) snapOwn = true;   // 我们在带这段滚动 → 所有权跟着跟随状态走
}
const snapSetters = new WeakMap();   // el -> 我们装上去的那个访问器（认亲：旧实例 dispose 迟到会把它删掉）
function takeOverSnap(el) {
  // 只在"当前装着的确实还是我们自己那个访问器"时早退：旧实例的 disposer 迟到会 delete 掉它，而 snapTaken
  // 还指着同一个元素 —— 原来会永远不再重装（吸底过滤静默失效，症状正是"新内容一出现就被吸到底"）。
  if (snapTaken !== null && snapTaken.el === el) {
    const cur = Object.getOwnPropertyDescriptor(el, "scrollTop");
    if (cur !== undefined && cur.set === snapSetters.get(el)) return;
  }
  ...
  const setter = function (value) {
    const own = snapOwn && state.autoFollow !== false && (followOn || glide !== null);
    if (!own) snapOwn = false;                     // 撒手（读者接管/切档/换会话）→ 位置立刻还给官方
    // 判据一：只挡「落到（夹住后的）底部」的写入 —— 而且目标值必须按 **floor 认，不能按 scrollHeight**：0.1.7 官方 toBottom 走的是
    // `ScrollFollow.jump → toBottom → ChatViewport.scrollToBottom`，写进来的是 `scrollHeight - clientHeight`
    // （一手调用栈 + 实测值 4474 = 5703 - 1229）。旧判据 `value >= scrollHeight - 1` 因此永不命中
    // （blocked 恒 0）：官方每来一坨字把我们的追赶滞后一次抹平，帧循环下一帧再回补，位置一上一下 ——
    // 就是「切会话后正文流式输出时整段对话上下跳」（steals 与 undo 成对增长）。
    const floor = Math.max(0, el.scrollHeight - el.clientHeight);
    // 判据二：接管期间（跟随中 / 滑行中）**一律**不许官方落底，不再受「离底 ≤600px」窗口限制 ——
    // 那个窗口本意是区分「官方小步追赶」与「有人真在跳转」，但本会话内的导航/换会话/切档已由 snapOwn=false
    // 覆盖；留着它，「单帧长高 >600px」和「滑行超过 600px」都会被当成跳转放行（一帧吸到底 / 半路被拽回底部）。
    if (!ownWrite && own && value >= floor - 1
      && (followOn || glide !== null || floor - el.scrollTop <= SNAP_UNDO_MAX)) { snapBlocked += 1; return; }
  ...
  snapSetters.set(el, setter);
  snapTaken = taken;
}
function guardSnap() {
  // 官方档 = 插件整体撒手：observer 还挂着，没有这道门就会在官方自己吸底之后把位置拽回我们旧的 followWrote。
  if (state.mode !== "fold") return;
...
function syncWindows(flow, running, collected, bodies, heights) {
  ...
  if (newestThink !== null && !newestThink.closest("[data-dshsf-folded]") && state.autoExpandReasoning !== false
    ...
    // 只有"还会变"的失败才值得续扫：用户收起过（keepOpen）/重试次数用满 = 永久不可达，
    // 再排下去就是"屏幕上什么都没变也每 300ms 全量重扫"。
    else if (newestThink.dataset.dshsfKeepOpen !== "1" && Number(newestThink.dataset.dshsfTries || "0") < 8) busyWork = true;
  }
  if (newestTool !== null && !newestTool.closest("[data-dshsf-folded]") && state.autoExpandTools !== false
    ...
    else if (newestTool.dataset.dshsfKeepClosed !== "1" && Number(newestTool.dataset.dshsfToolTries || "0") < 6) busyWork = true;
  }
...
/** 过程块：思考、工具卡（含 bash 卡）、上下文注入、内联过程。它们有自己的折叠/内联滚动，
 *  只用于诊断（probe().textWindowDetail）：确认某个残留的正文壳里是不是过程块。 */
const PROCESS_BLOCK = THINK + ", " + TOOL_CARD + ", [data-context-injection-body], [data-turn-process-inline]";
/** 撤销一个块的窗标记：短块不套壳时用（阈值由紧到松变化后，旧壳也要拆掉）。 */
function unwindowText(el) {
  if (el.dataset.dshsfTextwindow !== "1") return;
  el.removeAttribute("data-dshsf-textwindow");
  el.style.maxHeight = "";
  el.style.overflow = "";
  liveWindows.delete(el);
...
function syncTextWindows(flow, collected) {
  ...
  for (const turnRows of (collected ? collected.turns : collectTurns().turns).values()) {
    ...
    for (const row of assistantRows) {
      ...
      if (!body) continue;
      keptText = true;
      // 穿插正文**不套窗**：给官方 markdown 容器外套一层限高/内滚，会把它内部的布局压坏 —— 实测表现为
      // "文字被切一半"和"整块空白玻璃壳"。保留正文、直接自然排版才是最紧凑的。
      // 这里同时把历史版本留下的壳拆掉（unwindowText 幂等）。
      for (const child of Array.from(body.children)) unwindowText(child);
      row.removeAttribute("data-dshsf-tw");
    }
...
/* ---------------------------------------------------- 官方「对话显示」行 -- */
let nativeRowSeen = null;   // 上次标记过的那条官方行：还连着且标记还在，就不必每扫描全文档查按钮
function syncNativeRow() {
  const ours = document.querySelector(".dshsf-select");
  if (ours === null && document.querySelector("[data-dshsf-native-row]") === null) { nativeRowSeen = null; return; }
  if (ours !== null && nativeRowSeen !== null && nativeRowSeen.isConnected && nativeRowSeen.hasAttribute("data-dshsf-native-row")) return;
  for (const button of document.querySelectorAll('button[aria-haspopup="menu"]')) {
    ...
    if (!row) continue;
    if (ours !== null) { row.setAttribute("data-dshsf-native-row", ""); nativeRowSeen = row; }
    else { row.removeAttribute("data-dshsf-native-row"); nativeRowSeen = null; }
  }
}
...
function createSparkFx() {
  ...
  tone();
  // 注意：这一句**不能**缓存成一次性结果 —— 系统"减弱动态效果"中途变化必须立刻生效（test/spark.mjs 有断言）。
...
let readyTimer = 0;         // [streamfold] ready 那条日志的句柄：卸载后不该再打
let lastScan = 0;
let allowAnim = false;                       // 首屏不做动效（几十个过渡是加载卡顿主因）
let stoodDown = false;                       // 官方档位下是否已撒手（重活只做一次）
let observer = null;
...
function scanStructure() {
  ...
  finally { scanAnimateAll = false; }   // 官方档早退 / 抛错都不许把这个「本次扫描」标志留给下一次（否则切回折叠档整批历史行会播动画）
}
function scanStructureInner() {
  ...
  const flow = chatScope();
  if (state.mode !== "fold") {
    // 官方档位 = 插件整体撒手：不留任何我们的标记/自绘控件/跟随接管/置顶条（重活只在切过来的那一次做）。
    if (!stoodDown) { stoodDown = true; standDown(flow); }
    // busyWork 留着会让 schedule 每 300ms 自续（永久空转）；wasRunning 留着会在切回折叠时误触发"跑完回到提问处"。
    busyWork = false;
    wasRunning = false;
    syncNativeRow();                 // 设置页那一行仍由我们接管：两种模式都只显示一个下拉
    return;
  }
  stoodDown = false;
  if (!flow) return;
  ...
  if (settleHoldTurn !== null && stamp > settleHoldUntil) { settleHoldTurn = null; settleHoldUntil = 0; scanAnimateAll = true; }   // 兜底放行：滑行被别处取消时别把折叠无限期拖住（这仍是收尾折叠，照样播动画）
  if (wasRunning && !running) settleToPrompt(flow, collected.rows);   // 刚跑完这一轮（「移到提问处」在这里排一次）
  wasRunning = running;
  ...
  cost.counts.layouts = layoutReads;
  cost.scanMs += ms;
  scanSamples.push(ms);
...
function schedule() {
  ...
  requestAnimationFrame(() => {
    ...
    if (disposed) return;
    if (scanTimer) { clearTimeout(scanTimer); scanTimer = 0; }   // 同一个节流窗口会被排很多次：先撤旧的，只留一个待触发
    if (wait <= 0) { lastScan = now; scanStructure(); }
    else scanTimer = setTimeout(schedule, wait);         // 被节流挡下的这次变化，到点补扫一遍（别丢）
    if (busyWork) { if (scanTimer) clearTimeout(scanTimer); scanTimer = setTimeout(schedule, SCAN_INTERVAL); }   // 还有没做完的展开/收起，继续排队
...
function probe() {
  ...
  return {
    ...
    settleLog: settleLog.slice(-8),
    foldLog: foldLog.slice(-8),
    settleHold: settleHoldTurn,
    mainFollow: (() => {
      const el = mainScroller();
      if (!el) return null;
    ...
    // 每个正文小窗的现场：cls=class、len=可见文本长度、h=实测高度、sh=内容高度、process=是否含过程块
    // （len 小、h 大 = 空壳/被官方折叠的内容；确认"多出来的空白窗体"是什么就看这里）
    textWindowDetail: Array.from(document.querySelectorAll("[data-dshsf-textwindow]")).slice(-14).map((el) => ({
      cls: String(el.className || el.tagName).slice(0, 44),
      len: (el.textContent || "").trim().length,
      h: Math.round(el.getBoundingClientRect().height),
      sh: el.scrollHeight,
      process: el.querySelector(PROCESS_BLOCK) !== null ? 1 : 0,
    })),
    ...
    anim: {
      ...
      lastClose,           // 最近一次收起：anim=是否播了动画、以及每个判据当时的值
      lastReveal,
...
function startController() {
  ...
  const onUserActivate = (event) => {
    if (state.mode !== "fold" || synthetic || !(event.target instanceof Element)) return;   // 官方档位：不拦点击、不播揭示动效
    const flow = chatScope();
  ...
  const onBeforeMatch = (event) => {
    ...
    const skipped = state.mode === "fold" && target && (target.closest('[hidden="until-found"]') || target.closest("[data-dshsf-folded]"));
    if (!skipped) return;
  ...
  // 行子树一变就把该行的「有没有正文」缓存作废：React 复用同一行节点换内容时，轮号可能不变、
  // 只认轮号挡不住 —— 缓存说「没正文」会让承载正式回答的行被当过程行折掉（B3）。
  ...
  const dispose = () => {
    ...
    clearTimeout(runGraceTimer);
    clearTimeout(searchPrepTimer); searchPrepTimer = 0; searchPrepRunning = false;
    liveWindows.clear();
    unsubscribe();
    clearTimeout(timer);
    ...
    try { anchorStop(); } catch { /* ignore */ }   // 锚定接管（anchorRO + overflowAnchor）也必须回收
    ...
    document.documentElement.removeAttribute("data-dshsf-mode");    // 卸载后别把「折叠档」标记留在 <html> 上
    ...
    try {
      ...
      clearChips(document);
      for (const el of document.querySelectorAll("[data-dshsf-search]")) { el.removeAttribute("data-dshsf-search"); clearSkipHidden(el); }   // 归属感知：别把官方挂的那份也摘了、也别留僵尸标记
      for (const el of document.querySelectorAll("[data-dshsf-window], [data-dshsf-answer-sep]")) {
        el.removeAttribute("data-dshsf-window"); el.removeAttribute("data-dshsf-answer-sep");
        el.style.maxHeight = ""; el.style.overflow = "";
      }
      for (const el of document.querySelectorAll("[data-dshsf-jump]")) { try { el.remove(); } catch { /* ignore */ } }
    } catch { /* ignore */ }
  ...
  readyTimer = setTimeout(() => { if (disposed) return; try { console.info("[streamfold] ready", JSON.stringify(stats())); } catch { /* ignore */ } }, 2500);
  return dispose;
}
...
function DisplayRow() {
  ...
  return h(Row, { title: uiText("工作步骤展示", "Work details"), desc: uiText("选择希望看到多少工具调用细节；「折叠」是「流式折叠」提供的模式", "How much tool-call detail to show. Fold is provided by Streamfold.") },
...
function SettingsPage() {
  ...
  return h("div", { className: "dshsf-page" },
    ...
    h("div", { className: "dshsf-group" },
      ...
      h(Row, { title: "保留穿插正文", desc: "折叠时保留带正文的行（直接自然排版，不套窗；正式回答本身也不套窗）" },
        h(Toggle, { value: s.keepInterleavedText !== false, onChange: (v) => setSettings({ keepInterleavedText: v }) }))),
    h("div", { className: "dshsf-group" },
      h(Row, { title: "窗口高度", desc: "运行中思考小窗的最大高度（px）" },
...
function apply(ctx) {
  ...
  ctx.inject(["configForms"], (cctx) => {
    try {
      ...
      // 订阅必须能退订：否则每次重载/卸载都留一个常驻回调（闭包持有整个模块实例），
      // 且旧实例卸载后还会继续把用户的官方档位写回去。
      const offNative = nativeScope.subscribe(() => { if (disposed) return; adopt(); enforceNativeBaseline(); schedule(); });
      ctx.effect(() => () => { try { if (offNative) offNative(); } catch { /* ignore */ } }, "dsh-streamfold: native settings scope");
  ...
  if (ctx.slots) {
    ...
    ctx.slots.inject("settings.section", () => ctx.slots.register({ name: "settings.section", id: "streamfold", order: 42, label: () => uiText("流式折叠", "Streamfold") }, SettingsPage));
  }
}
exports.apply = apply;
