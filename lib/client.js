...
const RUNNING = '[data-state="running"], [data-state="preparing"], [data-streaming], [class*="_turnStatus"]';
...
function chatRows() {
  ...
  return Array.from(scope.querySelectorAll("[data-chat-flow-kind]"));
}
...
function collectTurns() {
  ...
  const mark = document.querySelector(RUNNING);
...
function syncTurns(flow, running, collected) {
  ...
  for (const [turn, turnRows] of turns) {
    ...
    // 正式回答 = 本轮最后一个「有正文」的 assistant-step 行；一个正文行都没有时才退回最后一个
...
function mainScroller() {
  ...
  const anchor = document.querySelectorAll("[data-chat-flow-kind]");
  const probe = anchor.length ? anchor[anchor.length - 1] : flow;
...
function DisplayRow() {
  ...
  return h(Row, { title: "工作步骤展示", desc: "选择希望看到多少工具调用细节；「折叠」由本插件接管（运行中的一轮看小窗，其余全折，折叠时官方停在完全展开）" },
