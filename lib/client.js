...
const API = "/streamfold/api/settings";
...
async function hostLoad() {
  try {
    ...
    if (data && data.ok && data.settings) {
      state = normalize({ ...DEFAULTS, ...data.settings });
      persistLocal();
      emit();
    }
  } catch { /* 无宿主路由时只用 localStorage */ }
}
function setSettings(patch) {
  ...
  emit();
  try {
    fetch(API, { method: "POST", credentials: "same-origin", headers: { "content-type": "application/json" }, body: JSON.stringify(patch) }).catch(() => {});
  } catch { /* ignore */ }
}
...
function displayChoice() { return state.mode === "fold" ? "fold" : (nativeValue === "normal" ? "normal" : "compact"); }
let nativePinned = false;   // 官方取值是否已校正（只做一次，避免写值 → 订阅 → 再写的回环）
/** 折叠模式必须让官方停在「标准」：官方 compact 会自己 hidden 掉过程行，与我们的折叠双重生效。 */
function enforceNativeBaseline() {
  if (nativePinned || state.mode !== "fold" || !nativeScope) return;
  nativePinned = true;
  const section = nativeScope.getSnapshot();
  const value = section && section.value ? section.value[NATIVE_FIELD] : undefined;
  if (value === "normal") return;
  try { nativeScope.set(NATIVE_FIELD, "normal"); } catch { /* ignore */ }
  nativeValue = "normal";
}
function chooseDisplay(choice) {
  if (choice === "fold") {
    // 以「标准」为底：官方 compact（compactTranscript）自己会把过程行 hidden 掉，
    // 和我们的折叠双重生效 —— 点开我们折叠条时官方那份 hidden 还在，等于打架。
    if (nativeScope) { try { nativeScope.set(NATIVE_FIELD, "normal"); } catch { /* ignore */ } }
    nativeValue = "normal";
    setSettings({ mode: "fold" });
    return;
  }
  setSettings({ mode: "native" });
  if (nativeScope) { try { nativeScope.set(NATIVE_FIELD, choice); } catch { /* ignore */ } }
  nativeValue = choice;
}
...
const CSS = [   // 本文件的样式表字符串（名字遮蔽了浏览器全局 CSS 对象，本插件已不再用 CSS.highlights）
  ...
  "[data-dshsf-jump]{position:sticky;bottom:6px;float:left;margin-left:6px;z-index:3;display:inline-flex;align-items:center;gap:4px;",
  "padding:2px 10px;border-radius:999px;border:.5px solid var(--dsw-alias-border-l2,rgba(127,127,127,.3));",
  ...
  ".dshsf-seg{display:inline-flex;border:.5px solid var(--dsw-alias-border-l2,rgba(127,127,127,.3));border-radius:8px;overflow:hidden}",
  ".dshsf-seg button{padding:4px 12px;font-size:12px;background:transparent;color:var(--dsw-alias-label-secondary);border:none;cursor:pointer;transition:background-color .16s ease,color .16s ease}",
  ".dshsf-seg button[data-active]{background:var(--dsw-alias-interactive-bg-active,rgba(127,127,127,.16));color:var(--dsw-alias-label-primary)}",
...
const RUNNING = '[data-state="running"], [data-streaming], [class*="_turnStatus"]';
const INDEPENDENT = new Set(["user", "steering", "system-prompt", "turn-error", "turn-max-tokens", "turn-tail", "turn-process", "compaction", "manual-compaction", "command", "request-prompt"]);
...
function hasText(row) {
  ...
  const result = body ? hasRowText(body) : false;
...
function assistantBody(row) {
  ...
  if (inline) return inline.parentElement;
  const md = row.firstElementChild;
  return md ? md.firstElementChild : null;
}
...
function scheduleSupersedeClose(root, ms) {
  ...
  root.dataset.dshsfCloseTimer = String(setTimeout(() => {
    ...
    const flow = root.closest(CHAT_FLOW);
    const row = root.closest(ROW);
    if (!flow || !row || !thinkOpen(root)) return;                       // 已经不在了
...
function wireWindow(el) {
  let jump = el.querySelector(":scope > [data-dshsf-jump]");
  if (!jump) {
    jump = document.createElement("button");
    jump.type = "button";
    jump.dataset.dshsfJump = "";
  ...
  el.addEventListener("scroll", () => {
    ...
    if (since > USER_SCROLL_MS) return;                                            // 没有输入信号 → 布局/锚定造成，不算读者
    const mine = el.dataset.dshsfMine;
    if (mine !== undefined && Math.abs(el.scrollTop - Number(mine)) <= 2) return;   // 我方推进造成的滚动，不算读者
    if (!atBottom(el)) el.dataset.dshsfFollow = "";
  }, { passive: true });
}
...
const cost = { frames: 0, tickMs: 0, scans: 0, scanMs: 0, lastMs: 0, maxMs: 0, phases: {}, counts: {}, worst: null, lastError: null };
...
function resetCost() { scanSamples.length = 0; cost.frames = 0; cost.tickMs = 0; cost.scans = 0; cost.scanMs = 0; cost.lastMs = 0; cost.maxMs = 0; cost.phases = {}; cost.counts = {}; cost.worst = null; cost.lastError = null; sparks.resetCost(); }
let followTickAt = 0;
let ticking = false;
...
function tickFollowInner() {
  ...
  for (const el of liveWindows) {
    ...
    plan.push({ el, jump: el.querySelector(":scope > [data-dshsf-jump]"), bottom, gap, follow, cur: el.scrollTop, want });
  }
  for (const item of plan) {
    ...
    if (item.jump) {
      ...
      const show = state.showJumpButton !== false && item.gap > 24 && (!item.follow || item.gap > 120);
      if (show !== item.jump.hasAttribute("data-dshsf-visible")) {
        if (show) item.jump.setAttribute("data-dshsf-visible", "");
        else item.jump.removeAttribute("data-dshsf-visible");
      }
    }
  }
...
function applyFold(row, fold, animate) {
  ...
  if (!animate || animBudget <= 0) {   // 配额用完 → 直接切（一次要动几十个元素时，动画本来也看不清）
    ...
    if (fold) row.setAttribute("hidden", SKIP);
    else if (isSkipped(row)) row.removeAttribute("hidden");
    return;
  }
  animBudget -= 1;
  ...
  if (isSkipped(row)) row.removeAttribute("hidden");   // 先摘掉，否则量到的高度是 0（内容根本没渲染）
...
function unfoldAll(container) {
  for (const row of container.querySelectorAll("[data-dshsf-row]")) {
    if (row.getAttribute("hidden") === SKIP) row.removeAttribute("hidden");
    row.removeAttribute("data-dshsf-folded");
    row.style.maxHeight = "";
    row.style.overflow = "";
  }
...
function searchPrep() {
  ...
  const flow = document.querySelector(CHAT_FLOW);
  if (!flow) return;
...
function collectTurns(flow) {
  const rows = Array.from(flow.children).filter((el) => el.hasAttribute && el.hasAttribute("data-chat-flow-kind"));
  const turns = new Map();
  ...
  return { rows, turns, lastTurn: rows.length ? rows[rows.length - 1].getAttribute("data-chat-turn") : null };
}

function syncTurns(flow, running, collected) {
  const { turns, lastTurn } = collected || collectTurns(flow);
  ...
  for (const [turn, turnRows] of turns) {
    ...
    const isRunningTurn = running && turn === lastTurn;
    const manual = manualState.get(turn);
    const assistantRows = turnRows.filter((row) => rowKind(row) === "assistant-step");
    const answerRow = assistantRows.length ? assistantRows[assistantRows.length - 1] : null;
    // 锻打锚点用**整行**（flowItem 在一轮里是稳定的），不用会随 React 重渲染被换掉的段落：
    // 段落一换，挂在它上面的 MutationObserver 和 Range 一起失效（颜色要等下一次扫描才回来 = 闪）。
    // 行稳定 → 观察者一直有效 → 结构一变就能在微任务里同步补色；内部再自己找"最后一个有字的文本节点"。
    ...
    for (const row of turnRows) {
      const fold = shouldFold && foldable.includes(row);
    ...
    if (anchor && chip.nextElementSibling !== anchor) flow.insertBefore(chip, anchor);
    const summary = [];
...
let lastAnnounceAt = 0;   // 上次补发 scroll 事件的时刻（压官方吸底用，不必每帧）
...
function mainFollowTick(dt) {
  if (state.autoFollow === false) return false;
  const scroller = mainScroller();
  ...
  const gap = floor - scroller.scrollTop;
  ...
  followWrote = scroller.scrollTop;
  ...
  // 它的标记能维持 SCROLL_SAMPLE_INTERVAL_MS(500ms)，所以没必要每帧派发——每帧派发会让官方和所有
  // 第三方监听每帧跑一次（含 React state 更新），400ms 一次足够压住它，开销降到 1/20 以下。
  const now = performance.now();
  if (now - lastAnnounceAt >= ANNOUNCE_MS) {
    lastAnnounceAt = now;
    announceCount += 1;
...
function onMainScroll(event) {
  ...
  if (followWrote !== null && Math.abs(el.scrollTop - followWrote) <= 2) return;      // 位置就是我写下的 → 我们自己造成的
  // 读者动过滚轮/触摸/翻页键/滚动条 → 立刻撒手
  if (performance.now() - scrollIntentAt <= USER_SCROLL_MS) { followOn = false; return; }
  // 没有输入信号：内容长度没变而位置变了 = 有人在跳转（官方导航轨、外部程序、选区自动滚动）→ 也算读者
  if (el.scrollHeight === followHeight) { followOn = false; return; }
  // 其余：展开/折叠让内容长高，浏览器滚动锚定顺手改了 scrollTop —— 这不是读者，保持贴底
}

let scrollerCache = null;   // 滚动容器几乎不变，但找它要逐级 getComputedStyle（每帧都做就是每秒上百次样式解析）
function mainScroller() {
  const flow = document.querySelector(CHAT_FLOW);
  if (scrollerCache && scrollerCache.isConnected && scrollerCache.contains(flow)) return scrollerCache;
  let node = flow ? flow.parentElement : null;
  while (node) {
    if (/(auto|scroll)/.test(getComputedStyle(node).overflowY)) { scrollerCache = node; return node; }
...
function syncWindows(flow, running, collected, bodies, heights) {
  ...
  const lastTurn = collected ? collected.lastTurn : collectTurns(flow).lastTurn;
...
function syncTextWindows(flow, collected) {
  ...
  for (const turnRows of (collected ? collected.turns : collectTurns(flow).turns).values()) {
    const assistantRows = turnRows.filter((row) => rowKind(row) === "assistant-step");
    const answerRow = assistantRows.length ? assistantRows[assistantRows.length - 1] : null;
    let keptText = false;
...
function syncNativeRow() {
  const ours = document.querySelector(".dshsf-seg");
  ...
  for (const button of document.querySelectorAll('button[aria-haspopup="menu"]')) {
    ...
    if (!/^(标准|紧凑|Standard|Compact)$/.test(text)) continue;
...
function createSparkFx() {
  ...
  function fieldFor(el, mode) {
    ...
    known = { el, canvas, ctx, w: 0, px: null, py: null, dpr: 0, parts: [], emit: 0, glow: 0, idle: 0, painted: false,
      ...
      burst: 0, nextBurst: 0.25 + Math.random() * 0.5, flash: 0, jitter: 0.5, level: 0, lastLevel: 0, flow: 0, emitEnd: 0,
      grad: null, gradW: 0,
  ...
  function drop(el) {
    ...
    if (!f) return;
    fields.delete(el);
    if (f.watch) { try { f.watch.disconnect(); } catch { /* ignore */ } f.watch = null; }
  ...
  /** 火星的"燃料"是小窗自己在滚：滚得越快越密，不滚就没有（没有基础量）。
   *  强度按**当前输出速度**归一化（输出快，同样的滚动量算"正常"；输出慢，一点点滚动就算"猛"），
   *  这样模型写得快还是慢，摩擦的观感都一致。短停 200ms 内留一点余温，免得逐帧微滚之间闪断。 */
  function readLevel(f, dt, top, height) {
    ...
    const flow = Math.min(Math.max(0, height - f.height) / per, 60);        // 当前输出速度（内容长得多快）
    const dir = clamp(moved / per / 8, -1, 1);
    f.top = top;
    f.height = height;
    f.flow = f.flow * 0.9 + flow * 0.1;                                    // 输出速度慢速 EMA（不跟单帧抖）
    const ref = clamp(f.flow * 0.6, 0.4, 6);                               // 这个输出速度下"跟得上输出"大约是滚多快
    const norm = speed / ref;                                             // 归一化：1 ≈ 正好跟上输出
    f.glow = f.glow * 0.88 + Math.min(norm, 3) * 0.25;
    f.idle = speed > 0.05 ? 0 : f.idle + dt;
    const level = f.idle > 200 ? 0 : Math.min(1.8, f.glow * 0.7);
    const resume = level > 0 && f.lastLevel === 0;       // 停过又滚起来：滚筒重新咬上金属
    f.lastLevel = level;
  ...
  function emit(f, s, level, dir, resume) {
    ...
    for (let i = 0; i < count; i += 1) spawn(f, dir, false);
    for (let i = 0; i < boom; i += 1) spawn(f, dir, true);
    ...
    for (let i = 0; i < ends; i += 1) spawnEnd(f, i % 2 === 0 ? -1 : 1, boom > 0);
  }
  function step(f, s) {
    const parts = f.parts;
  ...
  function draw(f) {
    ...
    const lit = Math.min(1.35, f.flash * 1.1 + Math.min(1, f.level) * (0.75 + 0.25 * f.jitter));
  ...
  function tick(dt) {
    ...
    for (const [el, f] of Array.from(fields)) {
      ...
      if (el === f.el && f.mode === "forge") {
        ...
        else { f.level = 0; plan.push({ f, hot: false }); }
...
function scanStructureInner() {
  ...
  if (!flow) return;
  const running = flow.querySelector(RUNNING) !== null;
  busyWork = false;
  closeBudget = 2;
  const tC = performance.now();
  ...
  const collected = collectTurns(flow);   // 一次就够：下面两处共用（chip 是 flow 的直接子节点但不带 data-chat-flow-kind，不影响这份快照）
...
function schedule() {
  ...
  requestAnimationFrame(() => {
    ...
    else setTimeout(schedule, wait);                     // 被节流挡下的这次变化，到点补扫一遍（别丢）
    if (busyWork) setTimeout(schedule, SCAN_INTERVAL);   // 还有没做完的展开/收起，继续排队
    ensureTickLoop();                                    // 跟随统一走帧循环，一帧只推进一次
  });
}
function stats() {
  const flow = document.querySelector(CHAT_FLOW);
  const { turns, lastTurn } = flow ? collectTurns(flow) : { turns: new Map(), lastTurn: null };
  const lastRows = turns.get(lastTurn) || [];
  return {
    mode: state.mode,
    running: flow !== null && flow.querySelector(RUNNING) !== null,
    lastTurn,
    windows: document.querySelectorAll('[data-dshsf-window="reasoning"]').length,
    openThinkAll: document.querySelectorAll(THINK + " " + THINK_BODY).length,
...
function probe() {
  const flow = document.querySelector(CHAT_FLOW);
  if (!flow) return { error: "no [data-chat-flow]" };
  const { rows, turns, lastTurn } = collectTurns(flow);
  ...
  return {
    ...
    officialView: nativeValue,          // 折叠模式下应为 "normal"（标准），不能是 compact
    ...
    mainFollow: (() => {
      ...
      if (!el) return null;
      return { on: followOn, gap: Math.round(el.scrollHeight - el.clientHeight - el.scrollTop), intentAgo: Math.round(performance.now() - scrollIntentAt), height: followHeight, wrote: followWrote === null ? null : Math.round(followWrote) };
    })(),
...
function startController() {
  ...
  // 不能按 [data-tool] 找：bash 卡走 tool.call.toolview 的 BashRow，根上没有 data-tool，只能按正文类名认。
  const onUserActivate = (event) => {
    if (synthetic || !(event.target instanceof Element)) return;
    const flow = document.querySelector(CHAT_FLOW);
    if (!flow || !flow.contains(event.target)) return;       // 点的不在对话流里（侧栏/设置）→ 不扫
  ...
  window.__dshStreamfold = { stats, probe, scan, set: setSettings, get: snapshot, chooseDisplay, resetCost, state: () => ({ ...state, nativeValue }),
    fx: sparks };   // 调参/排障入口：__dshStreamfold.fx 可实时调火花参数
...
function Seg(props) {
  return h("div", { className: "dshsf-seg" }, props.options.map((option) =>
    h("button", { key: option.value, type: "button", "data-active": props.value === option.value ? "" : undefined, onClick: () => props.onChange(option.value) }, option.label)));
}
function DisplayRow() {
  ...
  return h(Row, { title: "对话显示", desc: "标准保留全部过程；紧凑由官方折叠；折叠 = 本插件（运行中的一轮看小窗，其余全折）" },
    h(Seg, {
      value: choice,
      options: [{ value: "normal", label: "标准" }, { value: "compact", label: "紧凑" }, { value: "fold", label: "折叠" }],
      onChange: (value) => { chooseDisplay(value); setChoice(displayChoice()); },
    }));
}
...
/* ------------------------------------------------------------- apply -- */
function apply(ctx) {
  ctx.effect(() => injectStyles(), "dsh-streamfold: styles");
  ctx.effect(() => startController(), "dsh-streamfold: controller");
  if (ctx.settingsScope) {
    try {
      nativeScope = ctx.settingsScope.bind({ namespace: NATIVE_NS });
      const adopt = () => {
        const section = nativeScope.getSnapshot();
        const value = section && section.value ? section.value[NATIVE_FIELD] : undefined;
        if (value === "normal" || value === "compact") nativeValue = value;
      };
      adopt();
      enforceNativeBaseline();
      nativeScope.subscribe(() => { adopt(); schedule(); });
    } catch (error) { console.warn("[streamfold] native settings scope unavailable", error); }
  }
  hostLoad().then(() => { enforceNativeBaseline(); schedule(); });
  if (ctx.slots) {
