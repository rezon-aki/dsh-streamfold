...
function mainFollowTick(dt) {
  ...
  writeTop(scroller, scroller.scrollTop + step);
...
function userRows(flow) {
  const out = [];
  for (const el of flow.children) {
    if (el.hasAttribute && el.hasAttribute("data-chat-flow-kind") && el.getAttribute("data-chat-flow-kind") === "user") out.push(el);
  }
  return out;
}
function lastUserRow(flow) {
  const rows = userRows(flow);
...
function glideTick(scroller, floor, dt) {
  ...
  writeTop(scroller, scroller.scrollTop + Math.sign(dy) * step);
  followWrote = scroller.scrollTop;
  moveCount += 1;
  ensureTickLoop();
  return true;
...
function scanStructureInner() {
  ...
  if (state.mode !== "fold") {
    ...
    sparks.focus(null);
    sparks.forge(null);
    pinRows = flow ? userRows(flow) : [];   // 置顶条在两种显示模式下都要跟着滚（native 不折叠，但一样要钉）
  ...
  const collected = collectTurns(flow);   // 一次就够：下面几处共用（chip/固定层都不带 data-chat-flow-kind，不影响这份快照）
...
function startController() {
  ...
  for (const old of document.querySelectorAll("[data-dshsf-mainjump]")) { try { old.remove(); } catch { /* ignore */ } }   // 早期版本自绘的主窗口按钮（该属性已无生产者，留着只为清掉本会话遗留，下次发布可删）
