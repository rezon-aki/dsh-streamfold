...
function mainFollowTick(dt) {
  ...
  if (gap <= 25 && (gap <= 2 || mine)) followOn = true;
...
function lastUserRow(rows) {
  const list = rows || userRows();     // 扫描期已有 collected.rows：别再为此整篇查一遍会话行
...
function takeOverSnap(el) {
  ...
  const setter = function (value) {
    ...
    // 只挡「内容长高那一下」的抢跑：落到（夹住后的）底部、且**离底本来就不远**才丢；
    // 距离一律按「当前位置到 floor」算，导航/换会话那种从很远处整屏跳到底的写入照旧放行。
    // 目标值必须按 **floor 认，不能按 scrollHeight**：0.1.7 官方 toBottom 走的是
    ...
    if (!ownWrite && own && value >= floor - 1
      && floor - el.scrollTop <= SNAP_UNDO_MAX) { snapBlocked += 1; return; }
